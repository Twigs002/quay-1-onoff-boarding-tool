...
function readForUi_(ctx) {
  ...
  if (!isAdmin) {
    ...
    // Fail CLOSED (mirrors Programs.js `if (!me) return false`): a non-admin whose staff row has no
    // resolvable email sees NOTHING, never the full cross-entity pipeline or offboarding queue.
    var mine = {};
    if (email) {
...
function _bookedForResend_(isAdmin, email) {
  ...
  listOnboarding_().forEach(function (o) {
    ...
    // Fail CLOSED: a non-admin with no resolvable email is scoped to nothing (mirrors Programs.js).
