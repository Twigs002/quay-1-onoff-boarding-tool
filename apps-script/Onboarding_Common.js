...
function _ficaFollowUpDue_(o, now) {
  ...
  if (_isRemoved_(o)) return false;                 // admin-removed candidate - stop chasing
 ...
 * approved / provisioned / removed). Scheduled via setupTriggers() (Setup.js). Each send is recorded
 * on the candidate's communications log.
 * ---------------------------------------------------------------------------------------------- */

/** Whether a row is due the recurring weekly FICA reminder: contract sent, FICA not yet submitted,
 *  not approved / provisioned / removed, and a real (non-test) email. No time/once-only gate - the
 *  weekly trigger itself is the cadence. */
function _ficaWeeklyDue_(o) {
  if (!o.contract_emailed_at) return false;
  if (o.provisioned_at || o.approved_at) return false;
  if (_isRemoved_(o)) return false;
