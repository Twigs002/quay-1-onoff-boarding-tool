 ...
 * the induction packet, and the Aqua welcome pack.
 *
 * Why a dedicated store: logAudit_ (Util.js) only mirrors *_failed events to the Alerts tab and is
 * not keyed for read-back, so it cannot answer "what have we sent this person?". The candidate detail
 * page reads this tab back via readComms_ to show a per-candidate history.
 *
...
var COMMS_TYPE_LABEL = {
  ...
  aqua_welcome: 'Aqua welcome pack sent',
  aqua_induction: 'Aqua induction notice sent',
};

/** The Comms Log tab, created + header-seeded on first use (mirrors the create-on-demand pattern so a
 *  missing tab never throws the way tab_() would). */
function _commsTab_() {
  var ss = sheet_();
